...
import kotlinx.android.synthetic.main.activity_by_image.*
import android.graphics.Matrix
import android.util.Log
import android.widget.ImageView
import java.io.IOException

class ActivityByImage : AppCompatActivity() {
    ...
    private fun setPhoto(){
        ...
        try{
            ...
            val rotationInDegress : Int = exifToDegress(rotation)
            val matrix = Matrix()
            if(rotation != 0)matrix.preRotate(rotationInDegress.toFloat())
    ...
    private fun exifToDegress(exifOrientation: Int): Int {
